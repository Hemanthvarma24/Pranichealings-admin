...
// Healer form schema validation
const healerFormSchema = z.object({
...
type HealerFormValues = z.infer<typeof healerFormSchema>
...
function AddHealerForm() {
  ...
  const form = useForm<HealerFormValues>({
    resolver: zodResolver(healerFormSchema),
    defaultValues: {
      id: "",
      name: "",
      email: "",
  ...
  function onSubmit(data: HealerFormValues) {
    // Generate ID if not provided
    if (!data.id) {
      data.id = `HEAL${Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, "0")}`
    }

    // Create healer object with all data
    const healerData = {
      ...data,
      ...
      lastBooking: "New Healer",
    }

    try {
      // Get existing healers
      const existingHealers = JSON.parse(localStorage.getItem("healers") || "[]")

      // Add new healer
      localStorage.setItem("healers", JSON.stringify([...existingHealers, healerData]))

      // Redirect to healers list
      router.push("/user-entities/healers")
    } catch (error) {
      console.error("Error saving healer data:", error)
    }
  }
      ...
      <div className="flex flex-1 m-8">
        ...
        <main className="flex-1 p-6">
          <div className="container mx-auto max-w-5xl">
            <h1 className="mb-6 text-2xl font-semibold">Add Healer</h1>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <div className="flex justify-end gap-4">
                      <Button type="button" variant="outline" onClick={() => router.push("/user-entities/healers")}>
                        Cancel
                      </Button>
...
export default function AddHealerPage() {
  return (
